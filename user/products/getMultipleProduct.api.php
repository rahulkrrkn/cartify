...
if (isset($_SESSION["CFpublic"]["products"]["getData"]["searchCategoryId"])) {
    ...
    OR PV.VarentTypeValue1 LIKE ? 
    OR PV.VarentTypeValue2 LIKE ? 
    OR PV.VarentTypeValue3 LIKE ? 
    OR PV.ProductImage1 LIKE ?
ORDER BY 
    CASE 
...
";
    ...
    $search = $_SESSION["CFpublic"]["products"]["getData"]["search"];
    $parms = ["%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%", "%$search%"];
    $type = "ssssssssssss";
    
    $result = executeQuery(DBsqli(),  $query, $parms, $type);
} else {
    $result = executeQuery(DBsqli(),  $query);
};
