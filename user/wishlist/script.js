var LoadMultipleProduct = () => {
  let IDWhereInsert = "allProducts"
  loadingPage("start");
  sendAxios("/cartify/user/wishlist/getMultipleProduct.api.php",)
    .then((response) => { 
    ...
    <div class="wishlist" id="wishlist${
      e.ProductVariantsSNo
    }" onclick=" wishlistRemove(${e.ProductVariantsSNo},event)">
        ${
          e.WishListIs == true
...
function wishlistRemove(sNo,event) {
  wishlist(sNo, event)
