...
$ProductVaritySNo = $_POST['ProductVaritySNo'];
...
if (!isset($_FILES['ProductImageUplode0']['name']) || !isset($_FILES['ProductImageUplode1']['name']) || !isset($_FILES['ProductImageUplode2']['name'])) {
    sendResponse(false, ["Please Upload Atleast 3 Images", "message"]);
}
...
$stmt->bind_param(
    "iiissssssssssssssssssssssssssssssssssssssssssssssssssssssi",
    $ProductMRP,
    $ProductOfferPrice,
    $AvailableStock,
    $VarityValue1,
...
if ($stmt->execute()) {
    sendResponse(true, ["New Product $ProductName has been added successfully", "message"]);
} else {
   
    sendResponse(false, ["Failed to add product: " , "message"]);
}
